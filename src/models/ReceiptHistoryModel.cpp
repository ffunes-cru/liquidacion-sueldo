...
QVariant ReceiptHistoryModel::data(const QModelIndex &index, int role) const {
  ...
  case EsquemaCodigoRole:
    return r["esquema_codigo"];
  case MesRole:
    return r["mes"];
  case AnioRole:
    return r["anio"];
  ...
  case DatosJsonRole:
    return r["datos_json"];
  case FechaEmisionRole:
    return r["fecha_emision"];
  case NombreCompletoRole:
...
int ReceiptHistoryModel::employeeId() const { return m_employeeId; }

void ReceiptHistoryModel::setEmployeeId(int empId) {
  if (m_employeeId == empId)
    return;
  m_employeeId = empId;
...
int ReceiptHistoryModel::count() const { return m_data.size(); }
...
QVariantMap ReceiptHistoryModel::getReceipt(int id) const {
  QVariantMap rec = m_db->getReceipt(id);
  if (!rec.isEmpty()) {
    QString raw = rec.value("datos_json").toString();
    rec["datos_json_formatted"] = formatJson(raw);
  }
  return rec;
}

QString ReceiptHistoryModel::formatJson(const QString &rawJson) const {
